    ...
    <section class="mt-5 grid grid-cols-[1.85fr_0.95fr] gap-7 max-[1100px]:grid-cols-1">
      ...
      >
        <div class="rounded-[1.7rem] border border-amber-700/60 bg-[#180d0a] p-6">
          <div
            class="flex min-h-[270px] items-center rounded-[1.5rem] border border-amber-700/60 px-7"
          >
            ...
            <div
              class="ml-8 self-start rounded-xl bg-[#a43b2a] px-5 py-2 text-xl font-bold tracking-[0.16em]"
            >
              ON AIR
            </div>
          </div>
        ...
        <h3 class="mt-6 font-retro text-7xl leading-tight text-paper-50">
          {{ currentProgram.title }}
        </h3>
        <p class="mt-2 text-3xl text-paper-300">
          主播：{{ currentProgram.host }} · {{ currentProgram.duration }}
        </p>
        <p class="mt-2 max-w-3xl text-2xl text-paper-300">
          欢迎来到深夜电台，希望我的声音，能陪你度过这段漫长的夜晚。
        </p>
      ...
      <aside class="retro-card rounded-[2rem] p-7">
        ...
        <div
          class="mt-7 rounded-2xl border border-paper-700/55 bg-paper-100/75 px-4 py-4 text-xl text-paper-800"
        >
          听友留言：谢谢阿北，今晚终于没那么孤单。
        </div>
      </aside>
