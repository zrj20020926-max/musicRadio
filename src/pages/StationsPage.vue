    ...
    <section class="grid grid-cols-2 gap-8">
      <article class="rounded-[2rem] border border-paper-700/60 bg-[#22130f] p-8 shadow-soft">
        ...
        <div class="relative mt-6 rounded-[1.8rem] border border-paper-700/40 bg-paper-200 p-6">
          <div v-for="item in scheduleItems" :key="item.time" class="border-b border-paper-600/40 py-4 last:border-b-0">
          ...
          <div class="absolute -bottom-10 right-4 rounded-2xl border border-paper-700/50 bg-[#c8a663] px-7 py-5 text-center font-retro text-5xl text-paper-900">
            声音陪伴
            <br />
            每一天
          </div>
      ...
      <article class="rounded-[2rem] border border-paper-700/50 bg-paper-200 p-8 shadow-soft">
        ...
        <div class="relative mx-auto mt-10 h-28 w-[86%] rounded-3xl border border-paper-700/60 bg-[#1f120d] px-8 py-5">
          <div class="flex h-full items-center justify-between">
            <span v-for="n in 13" :key="n" class="h-10 w-[2px] bg-amber-500/70" :style="{ height: `${22 + (n % 3) * 10}px` }" />
          </div>
